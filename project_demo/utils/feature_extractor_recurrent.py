...
class FeatureExtractorRecurrent(nn.Module):
    '''
        FeatureExtractorRecurrent class that allows you to retain outputs of any layer
        and optionally store them as a list (capturing each iteration/pass of the module),
        or not (only retaining the final value emitted by the layer).
    ...
    '''    
    def __init__(self, model, layers, as_list=True, retain=True, detach=True, clone=True, device='cpu'):
        super().__init__()
        ...
        if self.as_list:            
            self._features = {layer: [] for layer in layers}
        else:
